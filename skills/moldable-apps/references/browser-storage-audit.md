...
| `mail` | `localStorage` | Persistent client cache for status/messages/drafts-related query data. | Anti-pattern: this mixes rebuildable provider data with durable drafts outside workspace-aware server storage. | Rely on TanStack Query memory cache, keep drafts under `getAppDataDir(workspaceId)`, and keep provider-derived messages/profile data under `getAppCacheDir(workspaceId)`. |
...
- `getAppCacheDir(workspaceId)` for server-side state that is safely rebuildable and may be purged
